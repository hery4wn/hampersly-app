                    ...
                    <form method="POST" action="{{ route('product.update', $product) }}" enctype="multipart/form-data">
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="description" name="description" 
                                      class="block mt-1 w-full border-gray-300 focus:border-[#FF9898] focus:ring focus:ring-[#FF9898] focus:ring-opacity-50 rounded-md shadow-sm">{{ old('description', $product->description) }}</textarea>
                        </div>
                        ...
                        <div class="mt-4">
                            <label for="image" class="block font-medium text-sm text-gray-700">{{ __('Ganti Gambar Produk (Opsional)') }}</label>
                            @if ($product->image_url)
                                <img src="{{ asset('storage/' . $product->image_url) }}" alt="{{ $product->name }}" class="h-24 w-24 object-cover rounded my-2 border border-gray-200">
                            @endif
